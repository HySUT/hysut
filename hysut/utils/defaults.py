...
class ModelSettings:
    ...
    def validate_solver(self, solver):
        ...
        if solver.upper() in cp.installed_solvers():
            ...
            warning.append(
                f"{solver} is not a valid solver or not installed on your machine. Default solver ({self.solver}) is used."
            )
            solver = self.solver
    ...
    def validate_log_path(self, path):
        ...
        if not isinstance(path, str):
            path = self.log_path
            warning.append(
                f"log_path should be str. Default log_path ({path}) is used."
            )
